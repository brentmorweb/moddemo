...
.pages-table tbody tr.is-selected {
        background: rgba(37, 99, 235, 0.12);
        box-shadow: inset 0 0 0 1px rgba(37, 99, 235, 0.18);
}

.pages-table tbody tr.is-hidden {
        display: none;
}
...
.pages-table tbody tr.empty-state {
        text-align: center;
        font-weight: 500;
        color: var(--pages-muted);
}

.pages-table tbody tr.empty-state td {
        padding: 60px 24px;
}
...
document.addEventListener('DOMContentLoaded', () => {
        const shell = document.querySelector('.pages-shell');
        if (!shell) {
                return;
        }

        const tableBody = shell.querySelector('.pages-table tbody');
        const rowPairs = Array.from(tableBody?.querySelectorAll('.page-row') || []).map((row) => ({
                row,
                checkbox: row.querySelector('.row-checkbox'),
        })).filter((pair) => pair.checkbox);

        const masterCheckbox = document.getElementById('masterCheckbox');
        const bulkBar = shell.querySelector('.bulk-actions-bar');
        const bulkCount = bulkBar?.querySelector('.bulk-count');
        const clearSelectionButton = bulkBar?.querySelector('.bulk-clear');
        const viewToggleButtons = Array.from(shell.querySelectorAll('.view-toggles .icon-button'));
        const tabButtons = Array.from(shell.querySelectorAll('.tab-button'));
        const chipButtons = Array.from(shell.querySelectorAll('.chip-button:not(.clear)'));
        const clearChipsButton = shell.querySelector('.chip-button.clear');
        ...
        emptyStateCell.colSpan = tableBody?.closest('table')?.tHead?.rows?.[0]?.cells?.length || 8;
        emptyStateCell.textContent = 'No pages match your filters yet.';
        emptyStateRow.appendChild(emptyStateCell);

        function getVisiblePairs() {
                return rowPairs.filter(({ row }) => !row.classList.contains('is-hidden'));
        }
        ...
        function refreshEmptyState() {
                ...
                emptyStateCell.colSpan = tableBody.closest('table')?.tHead?.rows?.[0]?.cells?.length || emptyStateCell.colSpan;

                if (visiblePairs.length === 0) {
                        if (!emptyStateRow.isConnected) {
                                tableBody.appendChild(emptyStateRow);
                        }
                } else if (emptyStateRow.isConnected) {
                        emptyStateRow.remove();
                }
        }

        function clearSelections() {
                rowPairs.forEach(({ row, checkbox }) => {
        ...
        function updateMasterCheckbox() {
                ...
                const selectedVisible = visiblePairs.filter(({ row }) => row.classList.contains('is-selected')).length;

                if (selectedVisible === 0) {
                        masterCheckbox.indeterminate = false;
                        masterCheckbox.checked = false;
                } else if (selectedVisible === visiblePairs.length) {
                        masterCheckbox.indeterminate = false;
        ...
        function updateBulkState() {
                const selectedCount = rowPairs.filter(({ row }) => row.classList.contains('is-selected')).length;
        ...
        if (masterCheckbox) {
                masterCheckbox.addEventListener('change', () => {
                        ...
                        const targetPairs = getVisiblePairs();

                        if (!shouldCheck) {
                                clearSelections();
                        } else {
                                targetPairs.forEach(({ row, checkbox }) => {
        ...
        const setViewMode = (button) => {
                ...
                if (view) {
                        shell.dataset.viewMode = view;
                } else {
                        delete shell.dataset.viewMode;
                }
        };

        if (viewToggleButtons.length) {
                const activeButton = viewToggleButtons.find((button) => button.classList.contains('active')) || viewToggleButtons[0];
                setViewMode(activeButton);
        ...
        function updateClearChipsState() {
                if (!clearChipsButton) {
                        return;
                }

                const hasActive = chipButtons.some((button) => button.classList.contains('active'));
                clearChipsButton.classList.toggle('is-disabled', !hasActive);
                clearChipsButton.setAttribute('aria-disabled', hasActive ? 'false' : 'true');
        }
        ...
        chipButtons.forEach((button) => {
                ...
                button.addEventListener('click', () => {
                        ...
                        updateClearChipsState();
                });
        });

        if (clearChipsButton) {
                clearChipsButton.setAttribute('aria-disabled', 'true');
                clearChipsButton.addEventListener('click', (event) => {
                        if (clearChipsButton.classList.contains('is-disabled')) {
                                event.preventDefault();
                                return;
                        }
        ...
        function applySearchFilter(query) {
                ...
                rowPairs.forEach(({ row, checkbox }) => {
                        const title = row.querySelector('.title-button')?.textContent.toLowerCase() || '';
                        const subtitle = row.querySelector('.subtitle')?.textContent.toLowerCase() || '';
                        const badge = row.querySelector('.badge')?.textContent.toLowerCase() || '';
                        const type = row.querySelector('.chip')?.textContent.toLowerCase() || '';
